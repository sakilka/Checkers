...
public class KingMove  implements ChessMove {
    ...
    public List<Dimension2D> potentialMoves(ChessSide side, ChessMoveHistory chessMoveHistory, Dimension2D currentPosition, ChessBoardPositions currentBoard) {
        ...
        if((side == WHITE && !chessMoveHistory.isCastlingWhiteDone())
                || (side == BLACK && !chessMoveHistory.isCastlingBlackDone())) {

            if(!kingNotMove(side, chessMoveHistory, currentPosition)
                    || (!leftRookNotMove(side, chessMoveHistory, currentBoard)
                    && !rightRookNotMove(side, chessMoveHistory, currentBoard))) {
                if(side == WHITE){
            ...
            if(kingNotMove(side, chessMoveHistory, currentPosition)
                    ...
                    && kingNotCheck(side, currentBoard, chessMoveHistory, currentPosition)
                    && !leftFieldsInAttack(side, currentBoard, chessMoveHistory, currentPosition)) {
            ...
            if(kingNotMove(side, chessMoveHistory, currentPosition)
                    ...
                    && kingNotCheck(side, currentBoard, chessMoveHistory, currentPosition)
                    && !rightFieldsInAttack(side, currentBoard, chessMoveHistory, currentPosition)) {
    ...
    private boolean kingNotMove(ChessSide side, ChessMoveHistory chessMoveHistory, Dimension2D currentPosition){
        if(side == WHITE) {
            if(currentPosition.width == 4 && currentPosition.height == 0) {
                return chessMoveHistory.getMoves().stream().noneMatch(move -> move.getPreviousPosition().width == 5
    ...
    private boolean leftRookNotMove(ChessSide side, ChessMoveHistory chessMoveHistory, ChessBoardPositions currentBoard){
        if(side == WHITE) {
            if(currentBoard.getPositions()[0][0] == ChessFigure.ROOK) {
                return chessMoveHistory.getMoves().stream().noneMatch(move -> move.getPreviousPosition().width == 1
                        &&  move.getPreviousPosition().height == 1);
            }
        } else {
            if(currentBoard.getPositions()[0][7] == ChessFigure.ROOK) {
                return chessMoveHistory.getMoves().stream().noneMatch(move -> move.getPreviousPosition().width == 8
                        &&  move.getPreviousPosition().height == 1);
            }
        }
    ...
    private boolean rightRookNotMove(ChessSide side, ChessMoveHistory chessMoveHistory, ChessBoardPositions currentBoard){
        if(side == WHITE) {
    ...
    private boolean kingNotCheck(ChessSide side, ChessBoardPositions currentBoard, ChessMoveHistory chessMoveHistory,
    ...
    private boolean leftFieldsInAttack(ChessSide side, ChessBoardPositions currentBoard, ChessMoveHistory chessMoveHistory,
                                       Dimension2D currentPosition){
    ...
    private boolean rightFieldsInAttack(ChessSide side, ChessBoardPositions currentBoard, ChessMoveHistory chessMoveHistory,
                                        Dimension2D currentPosition){
        for(int width = 0; width <8 ; width++) {
            for (int heigth = 0; heigth <8; heigth++) {
                if(side.oposite() == currentBoard.getSides()[width][heigth]) {
